...
function drawModules(buffer, size, pixels, frame, color) {
...
/** Eyes are drawn from the module center so the pair stays mirrored even at 32×32. */
function drawEyes(buffer, size, eyes, frame, color) {
  const scale = size / CANVAS;
  const targetSize = Math.max(1, Math.round(MODULE_SIZE * scale));
  for (const [column, row] of eyes) {
    const [x, y] = position(column, row, frame);
    const centerX = Math.round((x + HALF_MODULE) * scale - 0.5);
    const centerY = Math.round((y + HALF_MODULE) * scale - 0.5);
    fillTargetRect(
      buffer,
      size,
...
export function renderAvatarPng(descriptor, { size = DEFAULT_SIZE } = {}) {
  ...
  fill(rgb, hexToRgb(BACKGROUND));

  const frame = frameFor(descriptor);
  for (const layer of descriptor.layers) {
    drawModules(rgb, size, layer.pixels, frame, hexToRgb(layer.color));
  }
  drawEyes(rgb, size, descriptor.eyes, frame, hexToRgb(EYE));
